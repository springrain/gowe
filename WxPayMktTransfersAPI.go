...
func WxPayPromotionMktTransfers(wxPayConfig IWxPayConfig, body *WxPayPromotionMktTransfersBody) (*WxPayPromotionMktTransfersResponse, error) {
	// 业务逻辑
	bytes, err := wxPayDoWeChatWithCert(wxPayConfig, WxPayMchAPIURL+"/mmpaymkttransfers/promotion/transfers", body, 1)
	if err != nil {
		return nil, err
	}
	// 不返回sign不需要校验
...
func WxPayQueryMktTransfer(wxPayConfig IWxPayConfig, body *WxPayQueryMktTransferBody) (*WxPayQueryMktTransferResponse, error) {
	// 业务逻辑
	bytes, err := wxPayDoWeChatWithCert(wxPayConfig, WxPayMchAPIURL+"/mmpaymkttransfers/gettransferinfo", body, 0)
	if err != nil {
		return nil, err
	}
	// 不返回sign不需要校验
