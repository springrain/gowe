...
func WxMpQrCreateTemporary(ctx context.Context, sceneStr string, expireSeconds int) (*APIResult, error) {
	if len(sceneStr) < 1 {
	...
	apiurl := WxmpApiUrl + "/cgi-bin/qrcode/create?access_token=" + wxMpConfig.getAccessToken(ctx)
...
func WxMpQrCreatePermanent(ctx context.Context, sceneStr string) (*APIResult, error) {
	if len(sceneStr) < 1 {
		return nil, errors.New("sceneStr不能为空")
	}
